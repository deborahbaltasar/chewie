...
class DeviceController {
    async store(req, res){
        ...
        if(!checkUserAdmin) {
            return res.status(401).json({error: "Only admins can create a device."})
        }
        ...
        const meeting_room = await MeetingRoom.findOne({where: {id: req.body.meeting_room}})

        if (!meeting_room){
            return res.status(400).json({error: "Meeting Room does not exist"})
        }

        const category = await DeviceCategory.findOne({where: {id: req.body.category}})
    ...
    async show(req, res) {
        ...
        const categories = await DeviceCategory.findAll({attributes: ['id', 'name']})
        const devices = {
    ...
    async delete (req, res){
        ...
        if(!checkUserAdmin) {
            return res.status(401).json({error: "Only admins can delete a device."})
        }
