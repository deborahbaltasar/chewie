...
class InvoiceController {
    ...
    async store(req, res) { 
        ...
        const checkUserAdmin = await User.findOne({
          ...
          if(!checkUserAdmin) {
            return res.status(401).json({error: "Only admins can create an invoice"})
          }
        ...
        if (!emmiterExists) {
          return res.status(400).json({ error: 'Emmiter does not exist.' });
        }
  
      const { id, price, payment_date, issue_date, request_date, status } = await Invoice.create(req.body);
